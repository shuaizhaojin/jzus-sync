...
In Figure~\ref{fig:skew_int}, we plot the average estimated relative
...
Figure~\ref{fig:skew_minute}, we plot the relative clock skew between
...
Figure~\ref{fig:skew_cdc}, we plot the time difference between the
...
(see~Figure\ref{fig:cdf_duration}), which as Figure~\ref{fig:skew_int}
...
cave is less than 18 minutes (see Figure~\ref{fig:visitingfrequency}),
...
sub-cycles (see Figure~\ref{fig:searchlight}), resulting in a period
...
every $kp+2$ slots (see Figure~\ref{fig:searchlight+1}). We call this
...
Figure~\ref{fig:searchlight+1/2}). We call this modified scheme {\em Searchlight+1/2}.
...
\begin{table}[t]\footnotesize
   ...
   \begin{tabularx}{\columnwidth}{@{}l *{6}{>{\centering\arraybackslash}X}@{}}
      \toprule
      & \multicolumn{2}{c}{Searchlight-S} & \multicolumn{2}{c}{Searchlight-S+1} & \multicolumn{2}{c}{Searchlight-S+1/2} \\
      \cmidrule(r){2-3}
      \cmidrule(lr){4-5}
      \cmidrule(l){6-7}
...
latency by a small amount. Figure~\ref{fig:searchlight-cdf} shows that
...
particular day are shown in Figure~\ref{fig:trace}, where each cave is
...
In Figure~\ref{fig:a_c_break-25ms-5-orig}, we plot the cumulative
...
results in Figure~\ref{fig:a_c_break-25ms-1-orig}.  The discovery
...
mobile nodes spend in the caves in Figure~\ref{fig:cdf_duration},
...
Figure~\ref{fig:a_c_break-5ms-1-orig}. We see that as expected, the
discovery latency improves, but the latency increases to almost 5
times that of the original as in Figure~\ref{fig:a_c_break-25ms-5-orig}.
...
Figures~\ref{fig:a_c_break-5ms-1-dcs}
...
Figure~\ref{fig:improve}. An improvement of 90\% means that the
...
5\,ms. Figure~\ref{fig:small} shows the distribution of the discovery
...
$\pm100$~ppm~\cite{zhong2011demand}.  We showed experimentally that it
is possible for clocks to drift apart by 5\,ms in just one hour (See
Figure~\ref{fig:skew_cdc}). If the relative clock skew is 20\,ppm, and
since the average inter-arrival time of the mobile nodes in our
application is 15 minutes (See Figure~\ref{fig:visitingfrequency}),
...
protocol in Figure~\ref{fig:a_nc_break-5ms-1-searchlight}.  As
...
Figure~\ref{fig:a_c_random-5ms-1} shows the distribution of the
...
We have proved in the simulation experiment that with MASS, the discovery
latency of existing neighbor discovery protocols can be greatly improved.
However, it is not clear how it will perform on a real sensor network.
It is difficult and unrealistic to do such experiments at Mogao Grottoes,
since the current system is running and a sudden break is not allowed.
Thus we design a similar scenario to verify the performance of the proposed
MASS.
...
consists of a 32-bit ARM Cortex-M3 MCU and a low-power IEEE-compatible
radio transceiver AT86RF212 operating at 700/800/900 MHz with the 
transmission rate from 20 to 250~kbps. The distinguish feature of sensor 
nodes is the low power property, with the working and sleeping current 
30 mA and 1.2 $\mu$A respectively. A rechargeable 3.7V Li-Polymer battery
is used to power the sensor node.
...
\subsection{Clock Skew Compensation Interval}

The performance of proposed MASS is greatly affected by the accuracy of
clock skew compensation. Based on the obtained relative clock skew (as 
described in Sec.~\ref{sec:clock-drift}), we can get how much the active
slots of two nodes will drift after a period of time. And it is more
important to set an appropriate compensation interval to achieve the 
best performance. The compensation interval cannot be too small or big.
The error will be too large with a small interval, while the slot index
will not keep synchronized with a too large interval.

\todo{find ways to show the compensation interval selected.}

\subsection{Latency Reduction}
To investigate how MASS performs in reducing discovery latency, we
adopted the state-of-the-art Searchlight with 1\% duty cycle and 
