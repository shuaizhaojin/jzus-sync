...
\subsection{Heterogeneous Working Pattern}

Existing neighbor discovery protocols generally adopt a beacon-listen-beacon 
working pattern in active slots. This arrangement seems logical since it enables 
a pair of nodes to hear each other's beacon and achieve mutual discovery when 
their active slots overlap. However, with MASS, such working pattern will cause 
beaconing collision since the mobile node will perform slot index synchronization 
and slot alignment with the discovered static node. Thus, in our implementation, 
...
\begin{figure}[t]
   \centering
   \includegraphics[width=.9\columnwidth]{static/mockup-trace}
   \caption{Moving pattern in the mock-up scenario}
   \label{fig:mockup}
\end{figure}
...
the experiments on the third floor of a building. We used 10 rooms to 
simulate 10 caves and put one static node in each room. Then we invite 
volunteers to carry mobile nodes moving through different rooms based 
on a predefined moving pattern. The moving pattern (as shown in Fig.~\ref{fig:mockup}) 
is randomly produced following the moving pattern at Mogao Grottoes 
including the sequence of rooms and the staying time in each room. 
...
We adopted the state-of-the-art Searchlight-S with 1\% duty cycle and 
5~ms slot size. The experiments last three continuous days and there
are totally 271 discovery records with 32 volunteers invited. For 
comparison, the volunteers carry listening nodes and mobile nodes 
running with and without MASS following the same moving pattern.

Fig.~\ref{fig:latency-reduction} shows the cumulative distribution of the
discovery latency in the mock-up scenario compared with the simulation results
based on our custom simulator. We can see that with MASS the discovery latency 
can be greatly reduced. The average discovery latency can be reduced from 28~s
to 17~s and in 40\% cases the discovery latency is smaller than 1~s.

Fig.~\ref{fig:mock-day1} shows the reference selection process of the 
traces on the first day. We can see that after 50~minutes all the 9
static nodes can keep slot index synchronization with the node S10.
Fig.~\ref{fig:mock-priority} shows the priority of the ten static nodes.
