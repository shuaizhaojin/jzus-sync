...
static nodes situating at various rendezvous points. Among existing neighbor discovery protocols, probabilistic
...
collision since the mobile node will perform slot index synchronization and 
...
in active slots, the clock drift should not exceed 2\,ms in one
compensation interval, otherwise the discovery may fail even
...
four nodes where 1 node works as the static node while the other 3 
as the mobile nodes. All the four nodes run the Searchlight protocol with 
5\% duty cycle and 5~ms slot size. Mobile nodes will perform slot index 
synchronization and slot alignment with the static node after the first 
discovery. After 120~s, the 3 mobile nodes estimate the relative clock
skew with the static node. Based on the relative clock skew and the 1~ms
clock drift tolerance, each mobile node calculates its compensation interval.
...
practice due to measurement errors including clock skew estimation error,
...
synchronization can be extended to 2.3~hours, 5.25~hours and 6.15~hours compared to the
initial 0.77~hour, 1.15~hours and 1.75~hours without compensation for the
3 mobiles respectively.
